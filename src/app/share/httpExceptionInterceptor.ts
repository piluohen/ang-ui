...
export class HttpExceptionInterceptor implements HttpInterceptor {
  ...
  openMessageDialog(message: string , onOk?: () => void) {
      if (HttpExceptionInterceptor.isOpened) { return; }
  ...
  private handleData(event: HttpResponse<any> | HttpErrorResponse ) {
    switch (event.status) {
      ...
      case 401: // 未登录状态码
        ...
        this.openMessageDialog('登录超时，请重新登录', () => {top.window.location.href = environment.LOGIN_URL; });
        break;
      default:
  ...
  intercept( req: HttpRequest<any>, next: HttpHandler ): Observable<HttpSentEvent
    ...
    return handle.pipe(mergeMap((evt: any) => {
